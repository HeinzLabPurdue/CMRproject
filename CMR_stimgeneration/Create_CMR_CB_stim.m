% File: Create_CMR_CB_stim.m
%
% Update Apr 9 2021 - to make CMR_CB stimuli (CMRCB1)
% see Moore 1990 (Fig. 1); Niemeic et al 1992 (Table 1)
% tone in BP noise at tone, varied BWs, Mod and UnMod (REF) noise 
% Start
% training - easy
%  BWs = 500 Hz (within CB); 3000 Hz (outside CB)
%  Mod and REF
%  No = same as before
%  Tone levels = upper end of before (4 dB louder for 3000), keep same for
%  now.  MAKE THESE AS single conditions for now for easier analysis 
% LATER: what happened to random noises?
%
...
%% Noise BWs to useset to 1 ERB (either chin or human)
if strcmp(CMRcondition,'CMRcb1')
%     ERB_chin_Hz = 1200;
    % Human
...
if chin
    ...
    %     fprintf('...Using chin BWs (%.f Hz)\n',BWnoise_Hz)
    fprintf('...Using chin BWs\n')
else % human 
    error('human not set yet')
...
% levelVEC_tone_dBSPL = 75:4:75;  % ALL tone levels to include
...
% start with BBnoise,
noise_BB = randn(1,2*len_samples);  % independent samples eventually??
...
% modulate up to CF
noise_atCF = noise_LPF(1,:).*sin(2*pi*f_Hz*timevec_sec);

%% Modulate noise band
noise_atCF_mod = noise_atCF.*(1+sin(2*pi*fmod_Hz*timevec_sec));
...
for noiseIND=1:length(NoVEC_dBSPL_Hz)
    ...
    for toneIND=1:length(levelVEC_tone_dBSPL)
        ...
        %% Calibration (equal noise RMS for MOD and REF)
        ...
        noise_atCF= noise_atCF/rms(noise_atCF)*rms_noise_new;
        noise_atCF_mod= noise_atCF_mod/rms(noise_atCF_mod)*rms_noise_new;
        ...
        noise_atCF_rft=linear_window_waveform(noise_atCF,Fs_Hz,rft_noise_sec);
        noise_atCF_mod_rft=linear_window_waveform(noise_atCF_mod,Fs_Hz,rft_noise_sec);
        
        %% Create complete signals
        signal_REF = tone_rft + noise_atCF_rft;
        standard_REF = noise_atCF_rft;
        signal_MOD = tone_rft + noise_atCF_mod_rft;
        standard_MOD = noise_atCF_mod_rft;
        ...
        std_REF_fname = sprintf('%s_REF_BW%.f_No%.f_std.wav',CMRcondition,BWnoise_Hz,No_dBSPL_Hz);  % test condition1
        sig_REF_fname = sprintf('%s_REF_BW%.f_No%.f_T%.f_sig.wav',CMRcondition,BWnoise_Hz,No_dBSPL_Hz,level_tone_dBSPL);  % test condition1
        std_MOD_fname = sprintf('%s_MOD_BW%.f_No%.f_std.wav',CMRcondition,BWnoise_Hz,No_dBSPL_Hz);  % test condition1
        sig_MOD_fname = sprintf('%s_MOD_BW%.f_No%.f_T%.f_sig.wav',CMRcondition,BWnoise_Hz,No_dBSPL_Hz,level_tone_dBSPL);  % test condition1
        
        cd('new_signals')
        fprintf('Saving WAV files:\n  %s\n  %s\n  %s\n  %s\n  %s\n  %s\n',std_REF_fname,sig_REF_fname,std_MOD_fname,sig_MOD_fname)
        audiowrite(std_REF_fname,standard_REF,Fs_Hz)
        audiowrite(sig_REF_fname,signal_REF,Fs_Hz)
        audiowrite(std_MOD_fname,standard_MOD,Fs_Hz)
        audiowrite(sig_MOD_fname,signal_MOD,Fs_Hz)
        cd('..\')
        ...
        % MOD stimuli
        ax2=subplot(222);
        plot(timevec_sec*1000,signal_MOD,'r'); hold on; plot(timevec_sec*1000,standard_MOD,'b'); hold off
        xlim([0 dur_sec*1000])
        xlabel('Time (msec)')
        title(sprintf('MOD CONDITION:\nSTD: %s;    \nSIG: %s',std_MOD_fname,sig_MOD_fname),'interpreter','none')
        ax4=subplot(224);
        plot(freqvec_Hz/1000,20*log10(abs(fft(signal_MOD))),'r'); hold on; plot(freqvec_Hz/1000,20*log10(abs(fft(standard_MOD))),'b'); hold off
        xlim([0 10])
        ...
        disp('Playing Standard the Signal:  REF condition then MOD')
        sound([standard_REF zeros(size(signal_REF)) signal_REF zeros(1,3*len_samples) ...
            standard_MOD zeros(size(signal_MOD)) signal_MOD],Fs_Hz)
