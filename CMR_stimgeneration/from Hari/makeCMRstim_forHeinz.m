function x = makeCMRstim(fc, fs, fm, ofmbw, ofSNR, flankdist, flankbw,...
    condition, dur, ramps, rampn)
...
%   ramps: Duration of ramp on either end for tone (s)
%   rampn: Duration of ramp on either end for noise (s)
%
%
% OUTPUTS:
...
%% Make target
target = rampsound(sin(2*pi*fc*t), fs, ramps);
target = target / rms(target);

%% Make on-frequency masker
env = rampsound(0.5 + 0.5 * sin(2*pi*fm*t), fs, rampn);
env_dev = rampsound(0.5 - 0.5 * sin(2*pi*fm*t), fs, rampn);
% ERB to Hz conversion
bw = invcams(cams(fc) + ofmbw/2) - invcams(cams(fc) - ofmbw/2); 
ofm = makeNBNoiseFFT(bw, fc, dur, fs, rampn, 0)' .* env;
ofm = ofm / rms(ofm);
...
fmid = (fend + fstart)/2;
bw = fend - fstart;
flank_low = makeNBNoiseFFT(bw, fmid, dur, fs, rampn, 0)';
...
fmid = (fend + fstart)/2;
bw = fend - fstart;
flank_high = makeNBNoiseFFT(bw, fmid, dur, fs, rampn, 0)';
