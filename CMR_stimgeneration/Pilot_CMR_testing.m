...
% Last Modified by: Fernando Aguilera - July 13 2020 - Check for invalid
...
fprintf('\nThis demo is meant to guide you through a similar behavioral task in order for you to fully understand the actual testing task.\nNO DATA WILL BE RECORDED FOR THE DEMO BLOCK');
...
if demo_parameters(i,2) == 1 % REF
    ...
    if response == 1 || response == 2
        check = 1; % valid input
        if response == demo_parameters(i,3)
            fprintf('\nINCORRECT\n');
...
end % play REF, loudest tone level for demo block
end % play demo stimuli based on demo parameters
demoResponse = input('\nWould you like to repeat the demo block? (Y/N): ', 's');
end % demo block responses
